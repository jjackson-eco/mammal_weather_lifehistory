...
mam_res_names <- mnanom_5km_GAM %>% 
  ungroup() %>% 
  dplyr::select(ID, Order, Binomial, coef_temp, coef_precip) %>% 
  left_join(x = ., 
            y = dplyr::select(lpd_gbif, Binomial, gbif.species.tree, gbif.species.id.tree),
            by = "Binomial") %>% 
  group_by(gbif.species.tree) %>% 
  ...
  mutate(coef_temp_fac = cut(coef_temp, breaks = c(-2, seq(-0.5,0.5,length = 7), 2),
         ...
         coef_precip_fac = cut(coef_precip, breaks = c(-2, seq(-0.5,0.5,length = 7), 2),
                               labels = c("-2 to -0.5", "-0.5 to -0.33", "-0.33 to -0.17", "-0.17 to 0",
                                          "0 to 0.17", "0.17 to 0.33", "0.33 to 0.5", "0.5 to 2")),
         genus_label = gsub("_.*", "", gbif.species.tree),
         gbif.species.tree2 = gsub(" ", "_", gbif.species.tree))
  
## Dropping tips
mamMCC_weatheff <- keep.tip(mamMCC_pruned, mam_res_names$gbif.species.tree2)
...
tree_plotdat <- as_tibble(mamMCC_weatheff) %>% 
  full_join(x = ., y = mam_res_names, by = c("label" = "gbif.species.tree2")) %>% 
  as.treedata()
...
temp_tree <- ggtree(tree_plotdat, aes(colour = coef_temp_fac), layout = "circular", size = 0.6) +
    ...
    geom_tiplab(aes(label = genus_label), size = 2, colour = "black", offset = 5) +
...
ggdraw() +
  ...
  ggsave("plots/weather_pop_growth/mam_precip_tree.jpeg",
         height = 11, width = 11, units = "in", dpi = 400)
