...
mam_temp <- mnanom_5km %>% 
  dplyr::select(ID, ID_block, species = gbif_species, 
                Order, biome, Latitude, coef_temp, coef_precip) %>% 
...
set.seed(666)
temp_test <- brm(
  coef_temp ~ 1 + (1|gr(phylo, cov = A_temp)) + (1| species), 
...
precip_test <- brm(
  coef_precip ~ 1 + (1|gr(phylo, cov = A_precip)) + (1| species), 
...
# set.seed(666)
# temp_testOU <- brm(
#   coef_temp ~ 1 + (1|gr(phylo, cov = K)) + (1| species), 
#   data = mam_temp, family = gaussian(), 
#   data2 = list(K = K),
#   prior = c(
#     prior(normal(0, 0.25), class =  Intercept),
#     prior(exponential(0.1), class = sd, group = "phylo"),
...
# latitude
temp_lat <- brm(
  coef_temp ~ 1 + lat + (1|gr(phylo, cov = A_temp)) + (1| species), 
  data = mam_temp, family = gaussian(), 
  data2 = list(A_temp = A_temp),
  prior = c(
    prior(normal(0, 0.5), class =  Intercept),
...
mam2 <- filter(mam_temp, phylo != "Myotis_emarginatus")
mamMCC2 <- keep.tip(mamMCC_temp, mam2$phylo)
A_temp2 <- ape::vcv.phylo(mamMCC2)

temp_biome <- brm(
  coef_temp ~ 1 + biome + (1|gr(phylo, cov = A_temp2)) + (1| species), 
