...
##                                                         ##
##       Absolute weather effect with GAM - log normal     ##
##                                                         ##
...
## Predicting and plotting from the posterior of lognormal meta-regression models.
...
#_____________
## Best models
load("results/lognormal_models/temp_lh_uni.RData")
load("results/lognormal_models/precip_lh_uni.RData")

#______________________
## Model selection data
load("results/lognormal_models/temp_modcomp_lognormal.RData")
load("results/lognormal_models/precip_modcomp_lognormal.RData")
...
posterior_summary_longevity <- bind_rows(lapply(unique(preddat_lon$longevity), function(x){
  ...
  post_mn_temp = mean(exp(temp_pred_lon[,cpos]))
  post_mn_precip = mean(exp(precip_pred_lon[,cpos]))
  
  # prediction intervals - 80% from rethinking package
  cPI_temp = PI(exp(temp_pred_lon[,cpos]), prob = 0.8)
  cQuant_temp = quantile(exp(temp_pred_lon[,cpos]), c(0.025, 0.975))
  
  cPI_precip = PI(exp(precip_pred_lon[,cpos]), prob = 0.8)
  cQuant_precip = quantile(exp(precip_pred_lon[,cpos]), c(0.025, 0.975))
...
posterior_summary_litter <- bind_rows(lapply(unique(preddat_lit$litter), function(x){
  ...
  # posterior mean - Exponentiate to get back on response scale
  post_mn_temp = mean(exp(temp_pred_lit[,cpos]))
  post_mn_precip = mean(exp(precip_pred_lit[,cpos]))
  
  # prediction intervals - 80% from rethinking package
  cPI_temp = PI(exp(temp_pred_lit[,cpos]), prob = 0.8)
  cQuant_temp = quantile(exp(temp_pred_lit[,cpos]), c(0.025, 0.975))
  
  cPI_precip = PI(exp(precip_pred_lit[,cpos]), prob = 0.8)
  cQuant_precip = quantile(exp(precip_pred_lit[,cpos]), c(0.025, 0.975))
...
lon_temp_bin <- ggplot(longevity_binned_data, aes(x = lon_bin, y = mntemp)) +
  ...
  geom_point(aes(size = setemp), colour = temp_colour) +
  geom_smooth(data = posterior_summary_longevity,
...
lon_precip_bin <- ggplot(longevity_binned_data, aes(x = lon_bin, y = mnprecip)) +
  ...
  geom_point(aes(size = seprecip), colour = precip_colour) +
  geom_smooth(data = posterior_summary_longevity,
