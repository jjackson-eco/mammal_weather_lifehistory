...
##                                                 ##
##     CHELSA weather data extraction - 1979-1985  ##
##                                                 ##
##               April 23rd 2020                   ##
##                                                 ##
#####################################################
...
files_df <- tibble(files_temp, files_precip) %>% 
  ...
  filter(year >= 1979 & year <= 1985)         # <<<<<<<<-------------------------------------------- UNIQUE FOR THIS SCRIPT
...
starttime <- Sys.time()
mam_chelsa1979 <- bind_rows(lapply(X = 1:nrow(files_df), FUN = function(x){
...
##__________________________________________________________________________________________________
#### 3. Save ####
saveRDS(mam_chelsa1979, file = "lpi_weather_pilot/mam_chelsa1979.RDS") # On the UCloud
