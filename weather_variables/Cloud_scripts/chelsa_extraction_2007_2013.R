...
##                                                 ##
##     CHELSA weather data extraction - 2007-2013  ##
##                                                 ##
##               April 23rd 2020                   ##
##                                                 ##
#####################################################
...
files_df <- tibble(files_temp, files_precip) %>% 
  ...
  filter(year >= 2007 & year <= 2013)         # <<<<<<<<-------------------------------------------- UNIQUE FOR THIS SCRIPT
...
starttime <- Sys.time()
mam_chelsa2007 <- bind_rows(lapply(X = 1:nrow(files_df), FUN = function(x){
...
##__________________________________________________________________________________________________
#### 3. Save ####
saveRDS(mam_chelsa2007, file = "lpi_weather_pilot/mam_chelsa2007.RDS") # On the UCloud
