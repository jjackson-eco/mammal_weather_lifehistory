...
##                                                 ##
##     CHELSA weather data extraction - 1986-1992  ##
##                                                 ##
##               April 23rd 2020                   ##
##                                                 ##
#####################################################
...
files_df <- tibble(files_temp, files_precip) %>% 
  ...
  filter(year >= 1986 & year <= 1992)         # <<<<<<<<-------------------------------------------- UNIQUE FOR THIS SCRIPT
...
starttime <- Sys.time()
mam_chelsa1986 <- bind_rows(lapply(X = 1:nrow(files_df), FUN = function(x){
...
##__________________________________________________________________________________________________
#### 3. Save ####
saveRDS(mam_chelsa1986, file = "lpi_weather_pilot/mam_chelsa1986.RDS") # On the UCloud
