...
##                                                 ##
##     CHELSA weather data extraction - 1993-1999  ##
##                                                 ##
##               April 23rd 2020                   ##
##                                                 ##
#####################################################
...
files_df <- tibble(files_temp, files_precip) %>% 
  ...
  filter(year >= 1993 & year <= 1999)         # <<<<<<<<-------------------------------------------- UNIQUE FOR THIS SCRIPT
...
starttime <- Sys.time()
mam_chelsa1993 <- bind_rows(lapply(X = 1:nrow(files_df), FUN = function(x){
...
##__________________________________________________________________________________________________
#### 3. Save ####
saveRDS(mam_chelsa1993, file = "lpi_weather_pilot/mam_chelsa1993.RDS") # On the UCloud
